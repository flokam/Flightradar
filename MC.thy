...
definition WF_Kripke
  where \<open>WF_Kripke K \<equiv> (\<forall> x \<in> init K. \<forall> y. x \<rightarrow>\<^sub>i* y \<longrightarrow> y \<in> states K)\<close>
...
lemma AF_step_step': assumes "\<forall> y. x  \<rightarrow>\<^sub>i y \<longrightarrow> y \<in> AF f" shows  "x \<in> f"
  oops

lemma AF_step_star: "(\<forall> y. x  \<rightarrow>\<^sub>i* y \<and> y \<in> AF f) \<Longrightarrow> x \<in> AF f"
  by (simp add: state_transition_refl_def)

lemma AF_step_star': "(\<forall> y. x  \<rightarrow>\<^sub>i* y \<longrightarrow> y \<in> AF f) \<Longrightarrow> x \<in> AF f"
  by (simp add: state_transition_refl_def)

lemma AF_step_star'': "(\<forall> y. x  \<rightarrow>\<^sub>i* y \<longrightarrow> y \<in> f) \<Longrightarrow> x \<in> AF f"
  apply (simp add: state_transition_refl_def)
  by (simp add: AF_lem2a)

lemma AF_step_star_adv: "(\<forall> y. x  \<rightarrow>\<^sub>i* y \<longrightarrow> (\<exists> z. y  \<rightarrow>\<^sub>i* z \<and> z \<in> f)) \<Longrightarrow> x \<in> AF f"
  apply (simp add: state_transition_refl_def)
  apply (rule AF_step_star'')
  apply (simp add: state_transition_refl_def)
  oops

(* Only some y is not sufficient! *)
lemma AF_step_star': "x  \<rightarrow>\<^sub>i* y \<and> y \<in> AF f \<Longrightarrow> x \<in> AF f"
proof (simp add: state_transition_refl_def)
  show \<open>(x, y) \<in> {(x, y). x \<rightarrow>\<^sub>i y}\<^sup>* \<and> y \<in> AF f \<Longrightarrow> x \<in> AF f\<close>
  proof (erule conjE, erule converse_rtrancl_induct)
    show \<open>\<And> y. y \<in> AF f \<Longrightarrow> y \<in> AF f\<close>
      by assumption
  next show \<open>\<And> ya z.
       y \<in> AF f \<Longrightarrow>
       (ya, z) \<in> {(x, y). x \<rightarrow>\<^sub>i y} \<Longrightarrow>
       (z, y) \<in> {(x, y). x \<rightarrow>\<^sub>i y}\<^sup>* \<Longrightarrow> z \<in> AF f \<Longrightarrow> ya \<in> AF f\<close>
      apply (auto simp add: AF_step_step)
      apply (rule AF_step_star)
    oops
